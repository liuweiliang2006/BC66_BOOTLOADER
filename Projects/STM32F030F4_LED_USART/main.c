...
//		MB85RS16A_WRITE(BOOTFLAG_ADDR,(uint8_t*)&updata,sizeof(updata_t));  
    ...
    {			
		  ...
		  sim900a_gpio_init();
			...
			whiletime_out = 10;
			do{
//				result = sim900a_cmd_with_reply("AT+QBAND=4,1,3,5,8", "OK", NULL, GSM_CMD_WAIT_SHORT); //频段 china
				result = sim900a_cmd_with_reply("AT+QBAND=1,20", "OK", NULL, GSM_CMD_WAIT_SHORT); //频段 kenya
				if(result == FOUND)
				{
					//命令解析函数
